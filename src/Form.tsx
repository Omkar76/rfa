...
export const Form: FC<FormProps> = ({ closeForm }) => {
  ...
  // unlike other input fields form title needs to be immediately
  // written to localStorage since formlist depends on this value
  // if relied on useEfect that we have, the list will display
  // formName thats out of sync from whats entered in input tag
  const changeTitle = (title: string) => {
    const localForms = getLocalForms();
    const updateLocalForms = localForms.map((form) =>
      form.id === formData.id ? { ...form, title: title } : form
    );
  ...
  const deleteForm = (id: number) => {
    ...
    if (localForms.length > 1) {
      const updatedForms = localForms.filter((f) => f.id !== id);
      saveLocalForms(updatedForms);
      setFormData(updatedForms[0]);
    } else {
      const newForm = getNewForm();
